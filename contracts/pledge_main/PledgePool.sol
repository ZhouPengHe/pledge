...
contract PledgePool is ReentrancyGuard, SafeTransfer, multiSignatureClient{
    // 引入 SafeMath 库，用于防止整数溢出与下溢
    using SafeMath for uint256;
    // 引入 SafeERC20 库，用于安全地进行 ERC20 代币转账操作
    using SafeERC20 for IERC20;
    // 定义内部常量：计算用的默认精度，1e18 表示小数精度为 18 位
    uint256 constant internal calDecimal = 1e18;
    // 定义内部常量：用于利息与手续费计算的基础精度，1e8 表示小数精度为 8 位
    uint256 constant internal baseDecimal = 1e8;
    // 定义最小金额限制，默认值为 100 个代币（单位为 1e18）
    uint256 public minAmount = 100e18;
    // 定义时间常量：一年等于 365 天（Solidity 内置时间单位）
    ...
    // 定义默认状态，默认为 MATCH（撮合中）
    PoolState constant defaultChoice = PoolState.MATCH;
    // 定义全局暂停开关，true 表示暂停所有操作
    bool public globalPaused = false;
    // 定义 PancakeSwap 的路由器合约地址，用于执行兑换操作
    address public swapRouter;
    // 定义接收手续费的地址（可接收 ETH）
    address payable public feeAddress;
    // 定义预言机合约地址，用于获取资产价格
    IBscPledgeOracle public oracle;
    // 定义出借手续费比例（以 baseDecimal 为基准）
    uint256 public lendFee;
    // 定义借款手续费比例（以 baseDecimal 为基准）
    uint256 public borrowFee;

    // 每个池的基本信息
    struct PoolBaseInfo {
        // 池子的结算时间，用于计算利息或执行结算操作
        uint256 settleTime;
        // 池子的结束时间，达到该时间后池子将不再接受新的借贷操作
        uint256 endTime;
        // 池子的固定利率，单位为 1e8，例如 5% 就表示为 5e6
        uint256 interestRate;
        // 池子的最大资金供应量，即该池能承接的最大总额
        uint256 maxSupply;
        // 当前池中实际出借的资金总额
        uint256 lendSupply;
        // 当前池中实际借出的资金总额
        uint256 borrowSupply;
        // 池子的抵押率，单位为 1e8，例如 70% 表示为 7e7
        uint256 martgageRate;
        // 出借方使用的代币地址，例如 BUSD、USDT 等
        address lendToken;
        // 借款方使用的代币地址，例如 BTC、ETH 等
        address borrowToken;
        // 池子的当前状态，取值为 PoolState 枚举：MATCH, EXECUTION, FINISH, LIQUIDATION, UNDONE
        PoolState state;
        // sp_token 的 ERC20 地址，用于标记出借份额或权益份额
        IDebtToken spCoin;
        // jp_token 的 ERC20 地址，用于标记借款份额
        IDebtToken jpCoin;
        // 自动清算阈值，当池子抵押率低于该值时触发自动清算操作
        uint256 autoLiquidateThreshold;
    }
    // 定义所有基础池信息的数组（每个元素代表一个池子的基本配置）
    PoolBaseInfo[] public poolBaseInfo;

    // 定义结构体：用于记录每个池子的运行时数据信息
    struct PoolDataInfo {
        // 结算时的实际出借金额
    ...
    // 定义所有池子运行数据的数组（与 poolBaseInfo 一一对应）
    PoolDataInfo[] public poolDataInfo;

    // 定义结构体：记录每个借款用户在某个池子的借款信息
    struct BorrowInfo {
        // 当前借款的质押金额（抵押品数量）
        uint256 stakeAmount;
        // 借款结束后多余的退款金额
        uint256 refundAmount;
        // 是否已经退款，false 表示未退款，true 表示已退款
        bool hasNoRefund;
        // 是否已经认领（领取相关收益或返还），false 表示未认领，true 表示已认领
        bool hasNoClaim;
    }

    // 定义借款用户信息映射
    // 第一层 key 为用户地址，第二层 key 为池子索引（池ID）
    // 用于记录每个用户在不同池子的借款情况
    mapping(address => mapping(uint256 => BorrowInfo)) public userBorrowInfo;

    // 定义结构体：记录每个出借用户在某个池子的出借信息
    struct LendInfo {
        // 当前出借的质押金额（实际出借的资产数量）
        uint256 stakeAmount;
        // 超额退款金额（例如撮合未成功或溢出部分）
        ...
        // 是否已经退款，false 表示未退款，true 表示已退款
        bool hasNoRefund;
        // 是否已经索赔（领取本息或违约赔偿），false 表示未索赔，true 表示已索赔
        bool hasNoClaim;
    }
    // 定义出借用户信息映射
    // 第一层 key 为用户地址，第二层 key 为池子索引（池ID）
     ...
     * @param _swapRouter 交换路由合约地址
     * @param _feeAddress 手续费接收地址
     * @param _multiSignature 多重签名合约地址
     */
    constructor(
        address _oracle,
        address _swapRouter,
    ...
    /**
     * @dev 设置出借和借款手续费比例
     * @param _lendFee 出借手续费比例
     * @param _borrowFee 借款手续费比例
     * @notice 仅管理员可操作
     */
     ...
     * @dev 判断当前池子是否可以结算
     * @param _pid 池子索引（池编号）
     * @return 返回 true 表示可以结算，false 表示尚未到达结算时间
     */
     ...
     * @dev 判断当前池子是否可以结束
     * @param _pid 池子索引（池编号）
     * @return 返回 true 表示可以结束，false 表示尚未到达结束时间
     */
    ...
    /**
     * @dev 计算并扣除费用。根据传入的费率计算费用金额（= 金额 * 费率 / 基数），若费用大于 0，则调用 `_redeem` 从费用地址扣除相应数量的代币，最后返回扣费后的实际金额。
     * @param feeRatio 费用比例（基于 baseDecimal 的分母）
     * @param token 代币地址
     * @param amount 原始金额
     * @return 扣除费用后的金额
     */
    ...
    /**
     * @dev 获取代币交换路径
     * @param _swapRouter 路由合约地址（例如 PancakeSwap）
     * @param token0 起始代币地址（如果为0地址则表示使用WETH）
     * @param token1 目标代币地址（如果为0地址则表示使用WETH）
     * @return path 返回代币交换路径数组
     */
    ...
    /**
     * @dev 卖出指定数量代币（按指定输出计算输入）
     * @param _swapRouter 路由合约地址
     * @param token0 起始代币地址
     * @param token1 目标代币地址
     * @param amountout 目标输出代币数量
     * @return (输入数量, 实际获得输出数量)
     */
